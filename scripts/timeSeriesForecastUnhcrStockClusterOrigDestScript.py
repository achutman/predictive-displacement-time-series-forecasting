...
Clustering code @ https://github.com/achutman/predictive-displacement-cluster-INFORM-indicators
...
featsPass = ['RAstk','UcdpFat', 'UcdpEvt', 'Pts', 'HumRights', 
             ...
             'contig', 'dist', 'WdiUemD', 'WdiUrbD', 'WdiDepD', 'WdiCpiD']
...
###############################################################################
# (4) Select clusters of countries to model, 
# either manually or based on clustering INFORM indices
# Please refer to clustering code and outputs @https://github.com/achutman/predictive-displacement-cluster-INFORM-indicators
# Cluster 0
# countries = ['AFG', 'CAF', 'CIV', 'COD', 'SOM', 'TCD', 'UGA', 'YEM'] # Skipping 'SSD' because of data unavailability
# folderSave = 'Cluster0'
# Cluster 1 - Not very useful cluster in terms of analyzing displaced persons
# countries = ['BOL', 'ECU', 'JAM', 'KGZ', 'KHM', 'MAR', 'NIC', 'NPL', 'PER', 'SLV', 'TJK', 'UZB'] # Skipping 'CRI', 'CUB', 'DOM', 'PAN', 'VNM' because of data unavailability
# folderSave = 'Cluster1'
# Cluster 2
# countries = ['BGD', 'GTM', 'HND', 'HTI', 'IND', 'MMR', 'PAK', 'PHL']
# folderSave = 'Cluster2'
# Cluster 5
# countries = ['AGO', 'BFA', 'COG', 'COM', 'DJI', 'ERI', 'GHA', 'GMB', 'LBR', 
#               'MDG', 'MOZ', 'MRT', 'NAM', 'RWA', 'SEN', 'SLE', 'TZA', 'ZMB', 'ZWE'] 
#         # Skipping 'BEN', 'BLZ', 'BTN', 'BWA','CPV','DMA', 'FSM', 'GAB', 'GIN', 'GNB','GNQ', 'GUY', 'KIR', 'LAO', 'LSO', 'MHL', 'MNG', 'MWI', 'NRU','PNG','PRY','SLB', 'STP','SUR','SWZ','SWZ', 'TGO', 'TKM', 'TLS', 'TON','TUV','VUT', 'WSM',
# folderSave = 'Cluster5'        
# Cluster 9
# countries = ['COL', 'GEO', 'ISR', 'JOR', 'LBN', 'TUR'] # Skipping 'PSE', 'USA' because of data unavailability
# folderSave = 'Cluster9'
# Cluster 11
countries = ['BDI', 'CMR', 'ETH', 'KEN', 'MLI', 'NER', 'NGA', 'SDN']
folderSave = 'Cluster11'
# Cluster 16
# countries = ['IRQ','LBY','SYR']
# folderSave = 'Cluster16'
...
#pathSave = r'C:\Users\a.manandhar\OneDrive - Save the Children International\Documents\docs\PD phase two model\time series forecast\UNHCR\RAStkOrgDst\ClusterStkLagDestODtop10'
...
for orig in countries:    
    ...
    for idest, dest in enumerate(topNDestNames):    
        ...
        # Add Origin column
        dfOD['Orig'] = [orig for i in np.arange(dfOD.shape[0])]   
...
if (dfCatXY['Train'].sum()<MIN_TRAIN_SAMPLES):
    print('Undesirable to train - Training samples < %s'%(MIN_TRAIN_SAMPLES))    
elif (dfCatXY['Test'].sum()<MIN_TEST_SAMPLES):
    print('Undesirable to test - Test samples < %s'%(MIN_TEST_SAMPLES))    
else:    
    # Proceed   
    # # The following may not be relevant for clusters of countries      
    ...
    # Only proceed if there are MIN_TRAIN_SAMPLES of training samples and MIN_TEST_SAMPLES of test samples
    if (dfCatXY['Train'].sum()<MIN_TRAIN_SAMPLES):
        print('Undesriable to train - Training samples < %s'%(MIN_TRAIN_SAMPLES))        
    elif (dfCatXY['Test'].sum()<MIN_TEST_SAMPLES):
        print('Undesirable to test - Test samples < %s'%(MIN_TEST_SAMPLES))        
    else:    
        # Proceed 
        ...
        if SAVE_OUTPUTS:
            ...
            for orig in countries:
                os.mkdir(os.path.join(pathSave,folderSave,orig))                                                                 
                for nAhd in np.arange(N_AHEAD):
                    os.mkdir(os.path.join(pathSave,folderSave,orig,'%dyAhead'%(nAhd+1)))               
            ...
            for nAhd in np.arange(N_AHEAD):      
                ...
                savefig=True   
                pathSaveFull = os.path.join(pathSave,folderSave)
                print(pathSaveFull)                
                plot_per_nahead_out_cluster(dfCatXY,Yout_nahead,nAhd,LABELS,savefig,pathSaveFull)
